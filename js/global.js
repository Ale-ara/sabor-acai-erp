...
function escaparHtmlGlobal(valor){
    return String(valor ?? '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
function calcularDiasAssinatura(fimTeste){
    if(!fimTeste){
        return null
    }
...
function assinaturaEstaVencida(assinatura){
    if(!assinatura) return false

    if(['vencido', 'bloqueado'].includes(assinatura.status)){
        return true
    }
    ...
    return assinatura.fim_teste < hoje
}

function assinaturaDeveMostrarAviso(assinatura){
    if(!assinatura){
        return false
    }

    if(assinatura.status === 'teste'){
        if(sessionStorage.getItem('avisoTesteGratisOculto') === 'true'){
            return false
        }

        const ocultarEm =
        Number(sessionStorage.getItem('avisoTesteGratisOcultarEm') || 0)

        if(ocultarEm && Date.now() >= ocultarEm){
            sessionStorage.setItem('avisoTesteGratisOculto', 'true')
            return false
        }
    }
...
function obterTituloAvisoCobranca(assinatura){
    if(assinatura.status === 'bloqueado'){
        return 'Sistema bloqueado'
    }

    if(assinaturaEstaVencida(assinatura)){
        const dias =
        calcularDiasAssinatura(assinatura.fim_teste)

        if(dias !== null && dias < 0){
            return `Seu plano expirou há ${Math.abs(dias)} ${Math.abs(dias) === 1 ? 'dia' : 'dias'}`
        }
    ...
    if(assinatura.status === 'teste'){
        ...
        if(dias !== null && dias >= 0){
            return `Teste grátis: restam ${dias} ${dias === 1 ? 'dia' : 'dias'}`
        }

        return 'Teste grátis ativo'
    }

    return 'Aviso de cobrança'
}

function obterMensagemAvisoCobranca(assinatura){
    if(assinatura.status !== 'teste' && assinatura.mensagem_cobranca){
        return assinatura.mensagem_cobranca
    }
    ...
    if(assinatura.status === 'teste'){
        ...
        if(dias !== null && dias >= 0){
            return 'Você pode usar o sistema normalmente durante o período grátis.'
        }
    }

    return 'Regularize o pagamento para continuar usando o sistema.'
}

function montarAvisoCobranca(assinatura){
    if(document.getElementById('billing-warning')){
        return
    }

    const aviso = document.createElement('div')
    aviso.id = 'billing-warning'
    aviso.className = `billing-warning ${assinatura.status || 'teste'}`

    const titulo =
    escaparHtmlGlobal(
        obterTituloAvisoCobranca(assinatura)
    )

    const mensagem =
    escaparHtmlGlobal(
        obterMensagemAvisoCobranca(assinatura)
    )

    const linkPagamento =
    String(assinatura.link_pagamento || '').trim()

    const pixChave =
    String(assinatura.pix_chave || '').trim()
    .replace(/\\/g, '\\\\')
    .replace(/'/g, "\\'")
    ...
    aviso.innerHTML = `
        ...
        <div class="billing-warning-actions">
            ${
                linkPagamento
                ? `<a href="${escaparHtmlGlobal(linkPagamento)}" target="_blank" rel="noopener">Pagar agora</a>`
                : ''
            }

            ${
                pixChave
                ? `<button type="button" onclick="navigator.clipboard?.writeText('${pixChave}'); mostrarToast?.('Pix copiado','Chave Pix copiada.')">Copiar Pix</button>`
                : ''
            }
        </div>
    `
    ...
    document.body.classList.add('billing-warning-visible')
    ...
    if(assinatura.status === 'teste'){
        ...
        if(!ocultarEm){
            ocultarEm = Date.now() + 60000
            sessionStorage.setItem('avisoTesteGratisOcultarEm', String(ocultarEm))
        }

        const tempoRestante =
        Math.max(0, ocultarEm - Date.now())

        setTimeout(() => {
            ocultarAvisoTesteGratis()
        }, tempoRestante)
    }
}

function ocultarAvisoTesteGratis(){
    const aviso =
    document.getElementById('billing-warning')

    if(aviso?.classList.contains('teste')){
        aviso.remove()
    }

    document.body.classList.remove('billing-warning-visible')
    sessionStorage.setItem('avisoTesteGratisOculto', 'true')
}

async function verificarAssinaturaCliente(){
    if(window.location.pathname.endsWith('/dev-cobranca.html')){
        return
    }
    ...
    if(assinaturaDeveMostrarAviso(data)){
        const titulo =
        obterTituloAvisoCobranca(data)

        const mensagem =
        obterMensagemAvisoCobranca(data)

        if(data.status !== 'teste'){
            const referencia =
            `assinatura-${data.status || 'teste'}-${data.fim_teste || 'sem-data'}`
